...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'get_validation') {
        ...
        if (empty($code)) {
            ...
            $_REQUEST['redirect_url'] = $suffix . '?dispatch=auth.get_validation&expired=true';
        ...
        if (!empty($result)) {
            ...
            if ($count >= 3) {
                ...
                $_REQUEST['redirect_url'] = $suffix . '?dispatch=auth.login_form';
                return [CONTROLLER_STATUS_OK, $_REQUEST['redirect_url']];
            }
            fn_set_notification('E', __('error'), __('wrong_verification_code'));
            $_REQUEST['redirect_url'] = $suffix . '?dispatch=auth.get_validation&wrong_code=true';
            return [CONTROLLER_STATUS_REDIRECT, $_REQUEST['redirect_url']];
        }
    }
    //
    ...
    if ($mode== 'validation') {
        ...
        if (!empty($_REQUEST['redirect_url'])) {
            ...
            $redirect_url = fn_url('auth.get_validation' . !empty($_REQUEST['return_url']) ? '?return_url=' . $_REQUEST['return_url'] : '');
        }
        ...
        if (
            ...
            $_REQUEST['return_url'] = $suffix . '?dispatch=auth.get_validation';
            $_REQUEST['redirect_url'] = $suffix . '?dispatch=auth.get_validation';
            ...
            if (AREA == 'C') {
                if (empty($_REQUEST['quick_login'])) {
                    return array(CONTROLLER_STATUS_REDIRECT, 'auth.get_validation');
                } else {
                    Tygh::$app['ajax']->assign('force_redirection', fn_url('auth.get_validation'));
                    exit;
                }
            }
